...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    // thought of this solution first, but from what I understand,
    // it is redundant to transform a regular list to lazy list, since the whole list has already been calculated
//        {
//          for {
//            neighbour <- b.legalNeighbors
//          } yield (neighbour._1, neighbour._2 :: history)
//        }.to(LazyList)

    def helper(ns: List[(Block, Move)]): LazyList[(Block, List[Move])] = {
      ns match {
        case List() => LazyList()
        case (block, move) :: restNeighbours => (block, move :: history) #:: helper(restNeighbours)
      }
    }
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    for {
      neighbour <- neighbors if !explored.contains(neighbour._1)
    } yield neighbour
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    initial match {
      ...
      case _ =>
        ...
        newState <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
      } yield newState
        initial #::: from(newStates, explored ++ newStates.map(_._1))
    }
  }
